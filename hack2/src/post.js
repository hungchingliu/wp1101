...
function Post(props) {
  ...
  const getPostDetail = async() => {
    const{ data: {message: message, post: post} } = await instance.get('/postDetail', {
      params:{pid: pid}
    })
  ...
  const delPost = async () => {
    const{ data: {message: message, post: post} } = await instance.delete('/post', {
      params:{pid: pid}
    })
    console.log(["delpost", message]) 
    setTimeout(() => {
      props.navigate(-1)
    }, 300)
  }
